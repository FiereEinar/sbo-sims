...
export default function AddTransactionForm({
  ...
  const {
    ...
    resolver: zodResolver(
      mode === 'edit' ? transactionSchema : addModeFormSchema,
    ),
    defaultValues: {
      modeOfPayment: 'cash',
    },
  ...
  const onSubmitBatch = async (data: TransactionFormValues) => {
    try {
      ...
      if (invalidEntry) {
        setError('root', {
          message: 'Please select a category for all entries',
        });
        return;
      }
      ...
      if (invalidAmount) {
        setError('root', {
          message: 'Please enter a valid amount for all entries',
        });
        return;
      }
      ...
      <DialogContent className="sm:max-w-[500px] max-h-[85vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-2">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="modeOfPayment" className="text-sm">
              Mode of Payment:
            </label>
            <Controller
              control={control}
