...
export const admin_get_all_organizations = asyncHandler(async (req, res) => {
	const organizations = await req.OrganizationModel.find({}).lean();

	const enriched = await Promise.all(
		organizations.map(async (org) => {
			const userCount = await req.UserModel.countDocuments({
...
export const admin_create_organization = asyncHandler(async (req, res) => {
	...
	appAssert(typeof departments[0] === 'string', BAD_REQUEST, 'Invalid department');
	...
	const organization = new req.OrganizationModel({
		name, slug, governor, viceGovernor, treasurer, auditor, departments,
	});
	...
	const superAdminRole = await req.RoleModel.create({
		name: SUPER_ADMIN,
	...
	const hashedPassword = await bcrypt.hash(adminPassword, parseInt(BCRYPT_SALT));
	const adminUser = await req.UserModel.create({
...
export const admin_update_organization = asyncHandler(async (req, res) => {
	...
	const existingSlug = await req.OrganizationModel.findOne({
		slug,
