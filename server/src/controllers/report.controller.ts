...
export const get_monthly_report = asyncHandler(async (req, res) => {
  ...
  res.json(
    new CustomResponse(true, monthlyData, 'Monthly report (last 12 months)'),
  );
});
...
export const download_report_pdf = asyncHandler(async (req, res) => {
  ...
  // Mode of payment
  const mopAgg = await TransactionModel.aggregate([
    { $match: { organization: organizationId, semester, schoolYear } },
    {
      $group: {
        _id: '$modeOfPayment',
		...
		<table>
          ...
          monthly.length > 0
            ? monthly
                ...
                .join('')
            : '<tr><td colspan="3" style="color:#9ca3af;font-style:italic">No data for this period</td></tr>'
        }
			</tbody>
		</table>
	</div>
		...
		<table>
          ...
          categoryBreakdown.length > 0
            ? categoryBreakdown
                ...
                .join('')
            : '<tr><td colspan="7" style="color:#9ca3af;font-style:italic">No categories found</td></tr>'
        }
			</tbody>
		</table>
	</div>
