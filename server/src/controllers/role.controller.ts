...
export const createRole = asyncHandler(async (req, res) => {
  ...
  if (isDefault) {
    oldDefaultRole = await RoleModel.findOne({
      isDefault: true,
      organization: req.tenantContext!.organizationId,
    });
    if (oldDefaultRole) {
...
export const updateRole = asyncHandler(async (req, res) => {
  ...
  if (isDefault === true && !role.isDefault) {
    const oldDefaultRole = await RoleModel.findOne({
      isDefault: true,
      organization: req.tenantContext!.organizationId,
    });
    if (
      oldDefaultRole &&
      oldDefaultRole._id.toString() !== role._id.toString()
    ) {
      oldDefaultRole.isDefault = false;
      await oldDefaultRole.save();
...
export const deleteRole = asyncHandler(async (req, res) => {
  ...
  const usersWithRole = await UserModel.find({ rbacRole: roleID });
  appAssert(
