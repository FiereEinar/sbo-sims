...
import { DB_MODEL, originalDbName, SUPER_ADMIN } from '../constants';
...
export async function seedAdmin(): Promise<void> {
	try {
		...
		const UserModel = connection.model(DB_MODEL.USER, UserSchema);
		const RoleModel = connection.model(DB_MODEL.ROLE, RoleSchema);
