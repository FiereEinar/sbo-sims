...
const FIRSTNAME_PATTERNS = [
  'firstname',
  'first name',
  'first_name',
  'given name',
];
const LASTNAME_PATTERNS = [
  'lastname',
  'last name',
  'last_name',
  'surname',
 ...
 */
export const parseStudentBuffer = async (
  buffer: Buffer,
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        if (firstnameCol && row[firstnameCol])
          firstname = row[firstnameCol].toString().trim();
        if (lastnameCol && row[lastnameCol])
          lastname = row[lastnameCol].toString().trim();
        if (middlenameCol && row[middlenameCol])
          middlename = row[middlenameCol].toString().trim();
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        if (!studentID) {
          for (const key of Object.keys(row)) {
            if (
              STUDENT_ID_PATTERNS.some((p) => key.toLowerCase().includes(p))
            ) {
              studentID = extractStudentID(row[key]);
              if (studentID) break;
            }
        ...
        let course =
          courseCol && row[courseCol]
            ? row[courseCol].toString().trim().toUpperCase()
            : '';
        if (!course) {
          for (const key of Object.keys(row)) {
            if (COURSE_PATTERNS.some((p) => key.toLowerCase().includes(p))) {
        ...
        let gender =
          genderCol && row[genderCol] ? parseGender(row[genderCol]) : 'M';
        if (!genderCol) {
          for (const key of Object.keys(row)) {
            if (GENDER_PATTERNS.some((p) => key.toLowerCase().includes(p))) {
              gender = parseGender(row[key]);
        ...
        let section =
          sectionCol && row[sectionCol]
            ? row[sectionCol].toString().trim()
            : '';
        if (!sectionCol) {
          for (const key of Object.keys(row)) {
            if (SECTION_PATTERNS.some((p) => key.toLowerCase().includes(p))) {
