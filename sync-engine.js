    ...
    try {
      pullRes = await netRequest(
        `${atlasHealthUrl}/sync/pull?since=${lastSeq}&excludeClient=${clientId}&organizationId=${organizationId}`,
        {
          headers: {
            Authorization: `Bearer ${authCookie}`,
            'x-sync-secret': process.env.SECRET_ADMIN_KEY,
          },
        },
      );
    } catch (err) {
...
function setupIpc() {
  ...
  ipcMain.handle('sync:force-push', async (_event, payload) => {
    logToFile('[SyncEngine] Force Push initiated: ' + JSON.stringify(payload));
  ...
  ipcMain.handle('sync:force-pull', async (_event, payload) => {
    logToFile('[SyncEngine] Force Pull initiated: ' + JSON.stringify(payload));
    if (!cookie) throw new Error('Not authenticated');
